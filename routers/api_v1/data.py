...
@router.get("/trigger-n8n-img")
async def trigger_n8n_img(prompt: str,n8n_webhook_url: str, url: str):
    """
    ## 啓動 n8n 提示詞改圖
    ...
    """
    try:
        resp = requests.get(n8n_webhook_url, json={"prompt":prompt,"url":url}, timeout=10)
        resp.raise_for_status()
        return {"status": "success", "n8n_response": resp.json()}
    except requests.RequestException as e:
        return {"status": "error", "detail": str(e)}

@router.get("/trigger-n8n-prompt")
async def trigger_n8n_prompt(prompt: str,n8n_webhook_url: str, current_user: dict = Depends(get_current_active_user)):
    """
    ## 啓動 n8n 文字提示詞生圖
    - 參數: prompt (string)
